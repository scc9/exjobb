...
class LINUCB():
    ...
    def update_model(self):
        ...
        for arm in self.observed_articles:
            for x,y in zip(self.XX[arm],self.YY[arm]):
                #x.reshape(self.d,1)
                #print x
                self.update_A(arm,x)
                self.update_b(arm,x,y)
                self.update_w_A_inv(arm)
            self.XX[arm]=[] #RESET AFTER TRAINED
            self.YY[arm]=[] #RESET AFTER TRAINED
    ...
    def update_stats(self,arm,user_features,click):
        ...
        ''' #ONLY FOR 0 DELAY UPDATe
        self.update_A(arm,user_features)
        self.update_b(arm,user_features,click)
        self.update_w_A_inv(arm)
        '''
        self.XX[arm].append(user_features) #ONLY FOR DELAYED FB
        self.YY[arm].append(click)  #ONLY FOR DELAYED FB
